...
import { getInternalGenericOptions, getInternalDragOptions, getInternalCoordinatesOptions, getInternalDistanceAngleOptions } from '../utils/config';
import { InternalConfig, HandlerKey, UserHandlersPartial, InternalHandlers, UserHandlers, RecognizerClasses, UseGestureConfig, HookReturnType } from '../types';
 ...
 */
export function useGesture<Config extends UseGestureConfig>(handlers: UserHandlersPartial, config: UseGestureConfig = {}): (...args: any[]) => HookReturnType<Config> {
	/**
	 ...
	 */
	const { drag, pinch, ...restConfig } = config;
 ...
 * @returns
 */
function includeStartEndHandlers(handlers: UserHandlersPartial, handlerKey: HandlerKey, _nativeHandlers: UserHandlersPartial) {

...
export default abstract class Recognizer<T extends StateKey> {
	 ...
	 */
	constructor(protected readonly stateKey: T, protected readonly controller: Controller, protected readonly args: any[] = []) {} // eslint-disable-line
	...
	protected setTimeout = (callback: (...args: any[]) => void, ms: number = 140, ...args: any[]): void => {
		this.controller.timeouts[this.stateKey] = setTimeout(callback, ms, ...args);
	};

	// Convenience method to clear a timeout for a given gesture
	protected clearTimeout = () => {
		const v: NodeJS.Timeout | undefined = this.controller.timeouts[this.stateKey];
		v && clearTimeout(v);
	};
	...
	protected getMovement(values: Vector2, state: GestureState<T> = this.state): PartialGestureState<T> {
		 ...
		 */
		let movement = [_i0 !== false ? _m0 - _i0 : valueFn(initial)[0], _i1 !== false ? _m1 - _i1 : valueFn(initial)[1]] as Vector2;
	...
	protected fireGestureHandler = (forceFlag?: boolean): FullGestureState<T> | null => {
		...
		this.state.memo = newMemo !== undefined ? newMemo : this.state.memo;
